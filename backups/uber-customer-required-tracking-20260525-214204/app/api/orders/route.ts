...
export async function POST(request: Request) {
  try {
    ...
    const companyWorkerId = body.companyWorkerId ? Number(body.companyWorkerId) : null;
    ...
    for (const item of items) {
      ...
      if (selectedOptions.length !== modifierOptionIds.length) {
        return NextResponse.json(
          { error: "Una opcion seleccionada ya no esta disponible. Actualiza el pedido." },
          { status: 400 }
        );
      }
    ...
    if (orderSource === "online" && requestedDiscountCouponCode) {
      ...
      if (!coupon || !coupon.active) {
        return NextResponse.json(
          { error: "CupÃ³n invÃ¡lido o inactivo." },
          { status: 400 }
        );
      }
    ...
    if (companyWorkerId && !companyWorker) {
      return NextResponse.json(
        { error: "Trabajador empresa no encontrado." },
        { status: 404 }
      );
    }

    if (companyWorker && companyWorker.active === false) {
      return NextResponse.json(
        { error: "Trabajador empresa inactivo." },
        { status: 400 }
      );
    }

    if (companyWorker && companyWorker.companyCustomer?.active === false) {
      return NextResponse.json(
    ...
    if (
      companyWorker &&
      companyCustomerId &&
      Number(companyWorker.companyCustomerId) !== Number(companyCustomerId)
    ) {
      return NextResponse.json(
        { error: "El trabajador no pertenece a esta empresa." },
        { status: 400 }
      );
    }
    ...
    const order = await prisma.$transaction(async (tx) => {
      ...
      if (customerId && walletAmountUsed > 0) {
        ...
        if (companyWorker && walletAmountUsed > 0) {
          ...
          await tx.companyWorkerWalletTransaction.create({
            data: {
              companyWorkerId: companyWorker.id,
              companyCustomerId: companyWorker.companyCustomerId,
              type: "debit",
              amount: walletAmountUsed,
              workerBalanceAfter: updatedWorker.walletBalance,
              companyBalanceAfter: companyWorker.companyCustomer?.walletBalance || 0,
              reason: `Compra tÃ³tem pedido #${orderNumber}`,
              source: "worker_order_payment_totem",
            },
          });
        }
...
export async function PATCH(request: Request) {
  try {
    ...
    const approvalPin = String(body.approvalPin || "").trim();
    ...
    const requiresCompanyApproval =
      status === "ready" &&
      Boolean(
        existingOrder.companyCustomerId ||
          existingOrder.paymentMethod === "bank_transfer" ||
          existingOrder.orderSource === "company" ||
          existingOrder.orderSource === "company_worker" ||
          existingOrder.orderSource === "company_worker_totem"
      );

    if (requiresCompanyApproval) {
      const expectedPin = String(
        process.env.COMPANY_ORDER_APPROVAL_PIN || "1234"
      ).trim();

      if (!approvalPin || approvalPin !== expectedPin) {
        return NextResponse.json(
          { error: "Clave incorrecta para autorizar pedido empresa." },
          { status: 401 }
        );
      }
    }
