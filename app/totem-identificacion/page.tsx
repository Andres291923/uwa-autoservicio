...
export default function TotemIdentificationPage() {
  ...
  async function submit() {
    try {
      ...
      if (!sessionCode) {
        setMessage("Sesión no válida. Vuelve a escanear el QR.");
        return;
      }
      ...
      if (!response.ok) {
        setMessage(data.error || "No se pudo ingresar con esa cuenta.");
        return;
      }

      const customer = extractCustomer(data);
      const customerId = Number(customer?.id || data?.customerId || 0);

      if (!customerId) {
        setMessage("No pude obtener el cliente. Revisa la respuesta del login.");
        return;
      }
      ...
      if (!linkResponse.ok) {
        setMessage(linkData.error || "No se pudo conectar con el tótem.");
        return;
      }

      setMessage("Listo. Ya estás conectado al tótem. Puedes volver a la tablet.");
    } catch (error) {
      console.error(error);
      setMessage("Error al identificarte.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-8 text-zinc-950">
      <section className="mx-auto max-w-md rounded-[2rem] bg-white p-6 shadow-sm">
        ...
        >
          {loading ? "Conectando..." : "Conectar con tótem"}
        </button>
