...
function getCompanyIdFromUrl(request: Request) {
  const pathname = new URL(request.url).pathname;
  const parts = pathname.split("/").filter(Boolean);
  const index = parts.findIndex((part) => part === "company-customers");
...
export async function GET(request: Request) {
  try {
    const companyId = getCompanyIdFromUrl(request);
    ...
    console.error("COMPANY_HISTORY_ERROR", error);

    return NextResponse.json(
      {
        error: "No se pudo cargar historial de empresa.",
