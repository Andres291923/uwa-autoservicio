...
async function calculateOrderAmount(body: PreferenceBody) {
  ...
  if (body.flow === "personal_order" && body.customerId && requestedWalletAmount > 0) {
    ...
    const walletBalance = calculateWalletBreakdown(
  ...
  const amountToPay = Math.max(0, totalAfterDiscount - walletAmountUsed);
...
export async function POST(request: Request) {
  try {
    ...
    if (flow === "company_wallet_recharge") {
      ...
      if (amountToPay < 1000) {
        return NextResponse.json(
          { error: "La recarga minima es $1.000." },
          { status: 400 }
        );
      }
